...
public class CompUsableElectricChair : CompUsable
{
    ...
    public override IEnumerable<FloatMenuOption> CompFloatMenuOptions(Pawn pawn)
    {
        ...
        var cptu = parent.GetComp<CompPowerTrader>();
        if (cptu == null)
        {
            yield break;
        }

        var cf = parent.GetComp<CompFlickable>();
        if (cf == null)
        {
            yield break;
        }
        ...
        if (!cf.SwitchIsOn)
        {
            yield return new FloatMenuOption($"{FloatMenuOptionLabel(pawn)} (" + "SR_NoPower".Translate() + ")", null,
                MenuOptionPriority.DisabledOption);
            yield break;
        }
        ...
        if (!cptu.PowerOn)
        {
            yield return new FloatMenuOption($"{FloatMenuOptionLabel(pawn)} (" + "SR_NoPower".Translate() + ")",
        ...
        {
            yield return new FloatMenuOption(
                $"{FloatMenuOptionLabel(pawn)} (" + "SR_NoPrisoner".Translate() + ")", null,
                MenuOptionPriority.DisabledOption);
        }
    }
